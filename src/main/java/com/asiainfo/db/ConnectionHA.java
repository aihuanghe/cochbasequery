...
public class ConnectionHA {
    private final static Log log = LogFactory.getLog(ConnectionHA.class.getSimpleName());
    ...
    private static volatile ConnectionHA INSTANCE = null;
    ...
    public static ConnectionHA getInstance() {
        if(INSTANCE == null) {
            synchronized (ConnectionHA.class) {
                if (INSTANCE == null) {
                    INSTANCE = new ConnectionHA();
                }
            }
        }
        return INSTANCE;
    ...
    private static void readConf() {
        log.info("[ocnosql]start read conf file " + CommonConstants.FILE_NAME_HA + ".");
        FileInputStream fis = null;
        confProperty = new Properties();
        try {
            URL url = Connection.class.getClassLoader().getResource(CommonConstants.FILE_NAME_HA);
            log.info("[ocnosql]start read conf file " + CommonConstants.FILE_NAME_HA + "[" + url + "].");
            InputStream in = Connection.class.getClassLoader()
                    .getResourceAsStream(CommonConstants.FILE_NAME_HA);
            if (in == null) {
                throw new ClientRuntimeException("plz check if file "
                        + CommonConstants.FILE_NAME_HA + " in classpath!");
            }
    ...
    private void connect() throws ClientConnectionException {
        ...
        ////////////////////////////////////////////////////////
        conf = HBaseConfiguration.create();
        conf.addResource("hbase-resource/hbase-site-ha.xml");
